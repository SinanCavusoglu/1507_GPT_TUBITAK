...
def total_page_num(str_search, year):
    # Return max number of pages
    url = f"https://dergipark.org.tr/tr/search/1?q=%28title%3A+%22{str_search}%22+OR+running_title%3A+%22{str_search}%22+OR+journal%3A+%22{str_search}%22+OR+abstract%3A+%22{str_search}%22+%29+AND+%28pubyear%3A+%28%3E%3D{year}+AND+%3C%3D{year}%29%29&section=articles"
...
for str_search in words_list:
    article_dict = {"Year": [],"Page_Number": [], "Searching": [],"Article_Name": [], "Article_Journal": [], "Article_Volume": [], "Article_PDF_URL": []}
    for year in range(2010,2025):
        max_number = total_page_num(str_search, year)
        if max_number == 0:
            print("There is no such category in DergiPark")
            continue
        
        for page_num in range(1, max_number + 1):
            url = f"https://dergipark.org.tr/tr/search/{page_num}?q=%28title%3A+%22{str_search}%22+OR+running_title%3A+%22{str_search}%22+OR+journal%3A+%22{str_search}%22+OR+abstract%3A+%22{str_search}%22+%29+AND+%28pubyear%3A+%28%3E%3D{year}+AND+%3C%3D{year}%29%29&section=articles"
            page = requests.get(url, timeout = 999)
            soup = BeautifulSoup(page.text, "lxml")
            print(f"{str_search} - {page} - {page_num}/{max_number} - {year}")
            # ---------- BOXES ---------- #
